...
- **`src/database/db_manager.py`** (356 lines): Database management for users and newsletter subscribers using SQLAlchemy and Neon Postgres.
  - Classes: Subscriber, Recommendation, PortfolioHolding, TradeRecord, DailyPerformance, AllocationPlan …
...
- **`src/long_term/regime_classifier.py`** (239 lines): Long-cycle regime classification.
...
- **`src/screening/indicators.py`** (358 lines): Technical indicators module for stock analysis.
  - Functions: calculate_rsi, calculate_sma, calculate_ema, find_swing_lows, detect_volume_spike, calculate_support_strength, calculate_bollinger_bands, calculate_macd …
...
- **`src/screening/phase_indicators.py`** (884 lines): Phase-based technical indicators for the Quant Analysis Engine.
  - Functions: calculate_sma, calculate_slope, calculate_relative_strength, calculate_rs_slope, detect_volatility_contraction, find_base_high, find_pivot_high, calculate_volume_ratio …
...
- **`tests/test_quality.py`** (563 lines): Tests for data quality validation module.
  - Classes: TestDataQualityIssue, TestTickerQualityReport, TestFreshnessChecks, TestCompletenessChecks, TestAnomalyDetection, TestQualityScoring … | Functions: mock_db, quality_checker, fresh_fundamentals, stale_fundamentals, fresh_price_history, stale_price_history
...
- **`.github/workflows/daily_portfolio_operations.yml`** (72 lines): GitHub Actions workflow: 📈 Daily Portfolio Operations
- **`.github/workflows/daily_scan_with_email.yml`** (106 lines): GitHub Actions workflow: 📈 Daily Market Scan & Newsletter
- **`.github/workflows/daily_screening_git_storage.yml`** (50 lines): GitHub Actions workflow: 📂 Daily Data Persistence & Growth Scan
